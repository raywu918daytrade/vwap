...
⚠️ 跟 scripts/sync_market_db_from_hf.py 不同：那支是給 live_trader 開機
或人手動執行時同步本機 DB，預設整包 db/ 全拉。這支只給
.github/workflows/update_daily.yml 呼叫，刻意分成兩個檔案——GHA runner
每次都是全新機器（見下方動機說明），只需要「剛好夠讓增量/快路徑判斷
正確運作」的最小子集，不需要、也不該像本機同步那樣拉完整歷史。
