...
下游即時處理（_minute_tick_loop，每分鐘固定一次，用真實時鐘驅動）是兩條
分開的迴圈。不管這分鐘 WebSocket 有沒有推新訊息，_minute_tick_loop 都保證
每分鐘呼叫一次 on_minute（讀 db/m1_live/ 當下最新資料），讓 K 線與 VWAP
事件能穩定刷新。
...
# 觸發 on_minute 的時機：收盤後固定等 :05 秒是「最短」等待，之後改成看涵蓋率——
...
class FubonM1Collector:
    ...
    # 可能發生），每分鐘都要固定呼叫一次 on_minute，否則 K 線與 VWAP 刷新
    # 會因為某幾分鐘沒有新訊息就整段被跳過。
