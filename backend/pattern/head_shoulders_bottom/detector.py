...
class HeadShouldersBottomDetector(BasePatternDetector):
    ...
    def detect(self, df: pd.DataFrame, stock_id: str, timeframe: str) -> Optional[PatternResult]:
        ...
        for i in range(1, len(pivots) - 4):
            ...
            # 3. 頭部深度得分 (最高 15 分)
            score_head = min(15.0, head_depth * 75.0)
            ...
            if latest_close >= (slope * (n-1) + intercept) * 0.99:
                score_breakout = min(15.0, 7.5 + (latest_close - (slope * (n-1) + intercept)) / latest_close * 150.0)
            else:
                score_breakout = max(0.0, 7.5 * (latest_close - ph) / (pn2 - ph))
            ...
            total_score = float(min(100.0, score_shoulders + score_neckline + score_head + score_symmetry + score_breakout + score_recency))
