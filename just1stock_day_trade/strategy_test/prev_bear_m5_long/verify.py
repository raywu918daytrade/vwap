"""
同 prev_bear_m5_short 濾網，改做多（方向對照）。

昨陰實體≥5% → 今開高≥2% → 0050 開低 → 首 m5@09:05 跌
→ 09:05 收盤做多；TB 對稱 ±3% / 最多持有 30 分。
...
def _long_tb_m5(day_m5: pd.DataFrame, entry_ts: pd.Timestamp, entry: float) -> dict | None:
    """做多對稱 TB：+1 TP（價漲）/ -1 SL（價跌）/ 0 持滿。同根先 TP。"""
    if entry <= 0:
        return None
...
def run(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    print("prev_bear_m5_long（同 short 濾網，改做多對照）", flush=True)
    print(
    ...
    print(
        f"標籤: 做多 TB ±{TP_PCT:.0%} / 持有 {HOLD_M5_BARS} 根 m5（30分）",
        flush=True,
    )
    ...
    print("\n" + "=" * 56)
    print(f"做多 TB 三分類（±{TP_PCT:.0%} / 震盪30分）— 同濾網對照")
    print("=" * 56)
    _summarize(out)
...
def main():
    p = argparse.ArgumentParser(description="prev_bear_m5_long 同濾網做多對照")
